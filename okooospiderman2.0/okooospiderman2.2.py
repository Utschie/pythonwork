...
for i in datelist:#开启一个循环，保证爬取每天的数据用的UA，IP，账户都不一样
    ...
    header3['Referer'] = 'http://www.okooo.com/soccer/'#必须加上这个才能进入足球日历
    header3['Upgrade-Insecure-Requests'] = '1'#这个也得加上
    wangye = r.get('http://www.okooo.com/soccer/match/?date=' + i,headers = header3,)#进入指定日期，成功
    print('进入日期：'+ i)
    content1 = wangye.content.decode('gb18030')#取出wangye的源代码
    sucker1 = '/soccer/match/.*?/odds/'
    bisaiurl = re.findall(sucker1,content1)#获得当天的比赛列表
    for j in range(0,len(bisaiurl)):#逐个对每个比赛进行爬取
        william = r.get('http://www.okooo.com' + bisaiurl[0] + 'change/14/',headers = header3,)#打开威廉希尔
        content2 = william.content.decode('gb18030')
        sucker2 = bisaiurl[j] + 'change/.*?/'
        companyurl = re.findall(sucker2,content2)#从竞彩官方的源码中获取其他公司的链接
        for l in range(0,len(companyurl)):
            companyurl[l] = 'http://www.okooo.com' + companyurl[l]
        coprocess(companyurl)
    endpoint = time.time()
    print('日期：' + i + '，当天比赛爬取成功' + '用时：' + str(endpoint - startpoint) + '秒' + '\n')
    r.close()#关闭会话
