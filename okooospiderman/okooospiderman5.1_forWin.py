...
#另外，当出现Error"远程主机强制关闭了一个连接或者目标计算机积极拒绝"时应去除这个ip以提高成功爬取率。已通过copy库和copy函数完成————20181217
#有时格式不符的公司正是“威廉”，但也不总是。其实应该把格式不符的公司照抓
#代理换成了蜻蜓代理，每次提取10个ip，试一下。不好使————20181217
...
#经过一个月测试之后，基本上以后就是这个版本
#login函数可能还可以在优化修改，但是等到出错卡死的时候再5.1版本里做吧。
#把NoneType错误类型加了进去，写入日志。已完成————20181228
