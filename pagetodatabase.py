...
def pagetolist(url):
  ...
  type(company)#查看company的类型，是一个bs.element.resultset的对象
  company = str(company)#将company对象转换成字符串
  type(company)#查看company类型，是一个str
  ...
  type(rawresult)#rawresult是一个list,list里装的是30个元组，元组里装的3个字符串
  rawresult[3][2]#3号元组的2号字符串，即bet365
  ...
  with open("D:\\pythonlabor\\f.txt","w") as f:
    f.write(peilv)
    f.write('\n')
    f.write('\n')
    f.write(jishigailv)
