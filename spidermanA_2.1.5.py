...
#此爬虫程序用的是spiderman2.1.5版本,缺点是有时候会因为ip地址提取不出来而暂停
...
#另外还有一个缺点是即便100个文件换一个IP，IP还是会被封，可以考虑动态混拨，95%的有效率。
#下一步是做网页监控，即时更新数据集
import requests
...
start = time.time()
ip = requests.get('http://www.xdaili.cn/ipagent//newExclusive/getIp?spiderId=0a4b8956ad274e579822b533d27f79e1&orderno=DX20177263396OK6aAZ&returnType=1&count=1&machineArea=').content#API接口链接
ip = str(ip)
ip = re.search('(b\')(.*?)(\\\\n\')',ip)[2]
...
def pagetotxt(urlnum):
  ...
  for i in range(30,300,30):
      ...
      #time.sleep(random.uniform(0.01,0.05))#随机睡眠防止爬的过快
  ...
  address = 'D:\\lottotxt2\\{date_title}.txt'
...
for i in range(550300,700000,100):
    try:
      ip = requests.get('http://www.xdaili.cn/ipagent//newExclusive/getIp?spiderId=0a4b8956ad274e579822b533d27f79e1&orderno=DX20177263396OK6aAZ&returnType=1&count=1&machineArea=').content#API接口链接
      ip = str(ip)
      ip = re.search('(b\')(.*?)(\\\\n\')',ip)[2]
