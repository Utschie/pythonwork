#1.3版本用了手动分割进程的方式进行多进程，速度依然没有提高。
...
def pagetotxt(url):
  ...
  date = re.search('(<p class="game_time">比赛时间)(.*?)(</p>)',content
).group(2)
  date = date[0:10]
  ...
  bifen = re.search('(<strong>)(.*?)(</strong>)',content
).group(2)
...
if __name__ == '__main__':
  p1 = Process(target = proc(250000,250333))
  p2 = Process(target = proc(250333,250666))
  p3 = Process(target = proc(250666,251000))
  p1.start()
  p2.start()
  p3.start()
